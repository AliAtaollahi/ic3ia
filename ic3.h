...
private:
    ...
    struct MsatTermHash {
    size_t operator()(const msat_term& t) const noexcept {
        return static_cast<size_t>(msat_term_id(t));
    }
    };
    struct MsatTermEqual {
    bool operator()(const msat_term& a, const msat_term& b) const noexcept {
        return msat_term_id(a) == msat_term_id(b);
    }
    };
    std::unordered_map<msat_term, msat_term, MsatTermHash, MsatTermEqual> next2lbl_;
